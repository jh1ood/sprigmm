...
//  m_VBox_HScale.pack_start (m_Scrollbar);
  ...
  m_HBox_Combo.
    pack_start (*Gtk::manage (new Gtk::Label ("Scale Value Position:", 0)),
		Gtk::PACK_SHRINK);
  m_HBox_Combo.pack_start (m_ComboBox_Position);
  m_ComboBox_Position.signal_changed ().
    connect (sigc::mem_fun (*this, &Scales::on_combo_position));
  m_ComboBox_Position.set_active (0);	// Top

  //Digits:
  m_HBox_Digits.
    pack_start (*Gtk::manage (new Gtk::Label ("Scale Digits:", 0)),
		Gtk::PACK_SHRINK);
  ...
  m_adjustment_pagesize->signal_value_changed ().
    connect (sigc::mem_fun (*this, &Scales::on_adjustment2_value_changed));
  ...
  m_Button_Quit.signal_clicked ().
    connect (sigc::mem_fun (*this, &Scales::on_button_quit));
...
void
Scales::on_adjustment2_value_changed ()
{
