...
Sound::Sound(char *s, const char *r, const char *c)
{
	...
	if (err < 0) {
	    cout << "Sound::Sound: Start error: " << snd_strerror(err) <<
		endl;
	    exit(EXIT_FAILURE);
	}
    }
...
{
    static int icount = 0;
    cout << "asound_async_callback: channels = " << channels << ", icount = " << icount++ << endl;
			       ...
			       snd_pcm_hw_params_t * params)
{
    ...
    if (rrate != rate) {
	cout <<
	    "Sound::asound_set_hwparams: Rate does not match. Requested "
	...
	cout << "Sound::asound_set_hwparams: Unable to set buffer time " <<
	    buffer_time << "i for playback: " << snd_strerror(err) << endl;
	return err;
    }
    ...
    buffer_size = size;
    cout << "Sound::asound_set_hwparams: buffer_size = " << buffer_size <<
	endl;
	...
	cout << "Sound::asound_set_hwparams: Unable to set period time " <<
	    period_time << " for playback: " << snd_strerror(err) << endl;
	return err;
    }
			       ...
			       snd_pcm_sw_params_t * swparams)
{
	...
	if (err < 0) {
	    cout <<
		"Sound::asound_set_swparams: UUnable to set period event: "
		<< snd_strerror(err) << endl;
	    return err;
	}
    }
